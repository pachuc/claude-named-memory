...
Run `list-profiles.sh` via Bash. It emits one TSV line per profile: `name<TAB>words<TAB>mtime<TAB>preview`.
...
- Size: format the raw word count as e.g. `14.8k` for thousands, `812` for under 1000. Append ` (near limit)` if words > 18000 (compaction triggers at 20000).
